...
func TestMergeBoolFields(t *testing.T) {
	tests := []struct {
		name     string
		base     Config
		override Config
		...
		{
			name:     "HelixDBEnabled true overrides false",
			base:     Config{HelixDBEnabled: false},
			override: Config{HelixDBEnabled: true},
			want:     Config{HelixDBEnabled: true},
		},
		{
			name:     "HelixDBEnabled false does not unset true",
			base:     Config{HelixDBEnabled: true},
			override: Config{HelixDBEnabled: false},
			want:     Config{HelixDBEnabled: true},
		},
		{
			name:     "HelixDBAutoManage true overrides false",
			base:     Config{HelixDBAutoManage: false},
			override: Config{HelixDBAutoManage: true},
			want:     Config{HelixDBAutoManage: true},
		},
		{
			name:     "HelixDBAutoManage false does not unset true",
			base:     Config{HelixDBAutoManage: true},
			override: Config{HelixDBAutoManage: false},
			want:     Config{HelixDBAutoManage: true},
		},
		{
			name:     "zero value override preserves base",
			base:     Config{HelixDBEnabled: true, HelixDBAutoManage: true},
			override: Config{},
			want:     Config{HelixDBEnabled: true, HelixDBAutoManage: true},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.HelixDBEnabled != tt.want.HelixDBEnabled {
				t.Errorf("HelixDBEnabled = %v, want %v", got.HelixDBEnabled, tt.want.HelixDBEnabled)
			}
			if got.HelixDBAutoManage != tt.want.HelixDBAutoManage {
				t.Errorf("HelixDBAutoManage = %v, want %v", got.HelixDBAutoManage, tt.want.HelixDBAutoManage)
			}
		})
	}
}
